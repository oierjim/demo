    ...
    return (
        ...
        >
                ...
                {(filters, setFilters) => (
                    <>
                        <div className="col-12 md:col-2">
                            ...
                            <InputText value={filters.nombre} onChange={(e) => setFilters((prev: any) => ({...prev, nombre: e.target.value}))} placeholder={t('domain:animal.nombre')} className="p-inputtext-sm w-full" style={{ height: '39px' }} />
                        </div>
                        <div className="col-12 md:col-2">
                            <label className="block mb-2 font-semibold text-slate-600 text-xs uppercase tracking-wider">{t('domain:animal.raza')}</label>
                            <InputText value={filters.raza} onChange={(e) => setFilters((prev: any) => ({...prev, raza: e.target.value}))} placeholder={t('domain:animal.raza')} className="p-inputtext-sm w-full" style={{ height: '39px' }} />
                        </div>
                    </>
                )}
            </DataTableMaintenance.Filters>
            ...
            <DataTableMaintenance.Toolbar 
                showExport 
                newItemDefault={{ nombre: '', raza: '', peso: 0, altura: 0, fechaNacimiento: new Date() } as any} 
            />
                ...
                {(item, setItem, errors, isReadOnly) => (
                    <>
                        <div className="field mb-4">
                            ...
                            <InputText id="nombre" value={item.nombre || ''} onChange={(e) => setItem((prev: any) => ({ ...prev, nombre: e.target.value }))} required autoFocus disabled={isReadOnly} invalid={!!errors.nombre} />
                        ...
                        <div className="field mb-4">
                            ...
                            <InputText id="raza" value={item.raza || ''} onChange={(e) => setItem((prev: any) => ({ ...prev, raza: e.target.value }))} required disabled={isReadOnly} invalid={!!errors.raza} />
                        ...
                        <div className="formgrid grid">
                            ...
                            <div className="field col-6 md:col-3">
                                ...
                                <InputNumber id="peso" value={item.peso} onValueChange={(e) => setItem((prev: any) => ({ ...prev, peso: e.value || 0 }))} mode="decimal" minFractionDigits={1} maxFractionDigits={2} disabled={isReadOnly} />
                            </div>
                            <div className="field col-6 md:col-3">
                                <label htmlFor="altura" className="font-bold block mb-2">{t('domain:animal.altura')}</label>
                                <InputNumber id="altura" value={item.altura} onValueChange={(e) => setItem((prev: any) => ({ ...prev, altura: e.value || 0 }))} mode="decimal" minFractionDigits={0} maxFractionDigits={1} disabled={isReadOnly} />
                            </div>
                        </div>
                    </>
                )}
